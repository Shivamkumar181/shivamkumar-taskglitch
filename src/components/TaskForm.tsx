...
export default function TaskForm({ open, onClose, onSubmit, existingTitles, initial }: Props) {
  ...
  const handleSubmit = () => {
    ...
    // FIX: Include all required Task properties
    const payload: Omit<Task, 'id'> & { id?: string } = {
      title: title.trim(),
      revenue: typeof revenue === 'number' ? revenue : 0,
      timeTaken: safeTime,
      ...
      status: ((status || 'Todo') as Status),
      notes: notes.trim() || undefined,
      createdAt: initial?.createdAt || now, // Required field
      completedAt: initial?.completedAt || (status === 'Done' ? now : undefined), // Conditional field
      ...(initial ? { id: initial.id } : {}),
    };
